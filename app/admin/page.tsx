...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onSwitchToUserMode, currentUser }) => {
  ...
  const [testAlertPhone, setTestAlertPhone] = useState('14566');
  ...
  const filteredAssessments = assessments.filter((item) => {
    const matchesCategory = 
      ...
      item.stressCategory === selectedCategory ||
      (selectedCategory === 'MODERATE' && item.stressCategory === 'MEDIUM') ||
      (selectedCategory === 'MEDIUM' && item.stressCategory === 'MODERATE');
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-rose-800 bg-rose-100/80 px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
              ...
              {t('admin.clinicalBadge', 'Clinical Identity & Geolocation Dispatch Console')}
            </span>
            <span className="text-xs text-stone-400 hidden sm:inline">•</span>
            <span className="text-xs text-stone-500 font-medium hidden sm:inline">
              {t('admin.roleAdmin', 'Role: Master Clinician Admin')}
            </span>
          </div>
          <div className="flex items-center gap-2.5 sm:gap-3 pt-0.5">
            <Logo size="sm" showText={false} />
            <h1 className="text-xl sm:text-2xl font-bold text-stone-900 tracking-tight">
              {t('admin.commandCenter', 'Patient Identity & Geolocation Command Center')}
            </h1>
          </div>
          <p className="text-xs text-stone-500">
            {t('admin.commandDesc', 'Real-time identity verification, geographic coordinates tracking, and Twilio crisis SMS emergency dispatch.')}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          {onSwitchToUserMode && (
            ...
            >
              ...
              <span>{t('admin.userDashboard', 'User Dashboard')}</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
        ...
        <div className="bg-white border border-stone-200/80 rounded-2xl p-3.5 sm:p-5 shadow-2xs space-y-1">
          <div className="flex items-center justify-between text-stone-500">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">{t('admin.registeredPatients', 'Registered Patients')}</span>
        ...
        <div className="bg-white border border-stone-200/80 rounded-2xl p-3.5 sm:p-5 shadow-2xs space-y-1">
          <div className="flex items-center justify-between text-stone-500">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">{t('admin.assessments', 'Assessments')}</span>
        ...
        <div className="bg-[#F8D7DA] border border-red-200 rounded-2xl p-3.5 sm:p-5 shadow-2xs space-y-1">
          <div className="flex items-center justify-between text-[#842029]">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">{t('admin.highRiskAlerts', 'Alerts')}</span>
        ...
        <div className="bg-white border border-stone-200/80 rounded-2xl p-3.5 sm:p-5 shadow-2xs space-y-1">
          <div className="flex items-center justify-between text-stone-500">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">{t('admin.activeLocations', 'Locations')}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 bg-gradient-to-br from-stone-900 via-stone-850 to-slate-900 rounded-2xl p-5 text-white relative overflow-hidden shadow-inner flex flex-col justify-between min-h-[300px]">
            ...
            <div className="relative z-10 pt-2 border-t border-stone-800 flex flex-col sm:flex-row items-start sm:items-center justify-between text-[11px] text-stone-400 gap-2">
              ...
              <span>Emergency contact helpline: <strong className="text-rose-300">14566</strong></span>
            </div>
          </div>
          ...
          <div className="space-y-3 flex flex-col justify-between">
            ...
            <div className="bg-rose-50/80 border border-rose-200/80 rounded-2xl p-4 text-xs text-rose-900 space-y-1">
              ...
              <p className="text-[11px] text-rose-700 leading-relaxed">
                When a high-risk vocal anomaly occurs, GPS latitude & longitude are automatically embedded into the Twilio SMS payload sent to the user's emergency contact at <strong>14566</strong>.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-stone-100 p-1 rounded-xl border border-stone-200">
              ...
              >
                ...
                <span className="hidden sm:inline">{t('admin.viewCards', 'Cards')}</span>
              </button>
              <button
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder={t('admin.searchPatients', 'Search user by name, email, user ID, phone, city, or IP...')}
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
            ...
            {(['ALL', 'ORIGINAL', 'HIGH_RISK', 'MONITORED', 'ACTIVE', 'ADMIN'] as const).map((st) => (
              ...
              >
                ...
                {st === 'ALL' && t('admin.filterAll', 'All Patients')}
                {st === 'ORIGINAL' && t('admin.filterOriginal', 'Primary Patient')}
                {st === 'HIGH_RISK' && t('admin.filterHighRisk', 'High Risk')}
                {st === 'MONITORED' && t('admin.filterMonitored', 'Monitored')}
                {st === 'ACTIVE' && t('admin.filterActive', 'Active')}
                {st === 'ADMIN' && t('admin.filterAdmin', 'Admin')}
              </button>
            ))}
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs bg-stone-50/80 p-3 rounded-xl border border-stone-200/60">
                      ...
                      {(u.age || u.gender) && (
                        <div className="sm:col-span-2 pt-1 border-t border-stone-200/40 flex items-center gap-2 flex-wrap">
                          {u.gender && (
                            <span className="text-[10px] font-semibold text-stone-700 bg-stone-200/70 px-2 py-0.5 rounded-md">
                              Gender: {u.gender}
                            </span>
                          )}
                          {u.age && (
                      ...
                      <tr key={u.id} className={`transition-colors ${isOriginal ? 'bg-amber-50/20 hover:bg-amber-50/40' : 'hover:bg-stone-50/80'}`}>
                        ...
                        <td className="px-4 py-3">
                          ...
                          <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                            ...
                            {u.gender && (
                              <span className="text-[9px] font-medium text-stone-600 bg-stone-100 px-1.5 py-0.2 rounded">
                                {u.gender}
                              </span>
                            )}
                            {u.age && (
      ...
      <div className="bg-white border border-stone-200/80 rounded-2xl p-6 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-stone-500">
              Verify real-time Twilio cellular SMS delivery or send directly to designated emergency contact <strong className="font-mono text-stone-700">14566</strong>.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            <a
              href={`sms:14566?&body=${encodeURIComponent(
                '🚨 [MINDEASE ADMIN TEST CRISIS ALERT] Emergency communication link verified for high-risk monitoring. Helpline: 14566'
              )}`}
              target="_blank"
        ...
        <div className="p-4 sm:p-6 border-b border-stone-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1 flex-wrap">
              {['ALL', 'RISK', 'HIGH', 'MODERATE', 'LOW'].map((cat) => (
                <button
                  key={cat}
            ...
            <tbody className="divide-y divide-stone-100">
              {filteredAssessments.length === 0 ? (
                ...
                filteredAssessments.map((item) => {
                  const isRisk = item.stressCategory === 'RISK';
                  const isHigh = item.stressCategory === 'HIGH';
                  const isMed = item.stressCategory === 'MODERATE' || item.stressCategory === 'MEDIUM';
              ...
              <div className="bg-stone-50 rounded-2xl p-4 border border-stone-200/80 space-y-3">
                ...
                <div className="space-y-1.5 text-xs">
                  ...
                  {selectedUserForModal.gender && (
                    <div>
                      <span className="text-[10px] text-stone-400 uppercase font-semibold">Gender:</span>
                      <div className="font-semibold text-stone-800">{selectedUserForModal.gender}</div>
                    </div>
                  )}
                  {selectedUserForModal.age && (
                    <div>
                      <span className="text-[10px] text-stone-400 uppercase font-semibold">Age:</span>
                      <div className="font-semibold text-stone-800">{selectedUserForModal.age} years old</div>
                    </div>
                  )}
                </div>
              </div>
