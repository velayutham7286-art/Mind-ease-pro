...
- **General Counseling** (powered by \`gemini-3.8-flash\`) for daily stress management and emotional validation
...
How can I help you find calm right now?`,
  ...
  const handleSendMessage = async (customPrompt?: string) => {
    ...
    const userMsg: MessageItem = {
      ...
      role: 'user',
      content: textToSend,
      model: taskType === 'complex' ? 'gemini-3.1-pro-preview' : taskType === 'fast' ? 'gemini-3.1-flash-lite' : 'gemini-3.8-flash',
      taskType,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
    };
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            language: language,
            languageName: currentLanguageInfo.name
          }
        })
      });
      ...
      const assistantMsg: MessageItem = {
        ...
        model: data.model || (taskType === 'complex' ? 'gemini-3.1-pro-preview' : taskType === 'fast' ? 'gemini-3.1-flash-lite' : 'gemini-3.8-flash'),
      ...
      const errorMsg: MessageItem = {
        id: `err-${Date.now()}`,
        role: 'model',
        content: "I'm temporarily experiencing a connection pause. Let's take a slow, grounding breath together: Inhale for 4 seconds, pause for 7, and exhale for 8. Immediate 24/7 crisis support is always open at 14566.",
        model: 'offline-coping-engine',
        taskType,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        suggestions: ['Start 4-7-8 Breathing Guide', 'Call Helpline 14566']
      };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {isSyncingWithFirestore && (
              <span className="text-[11px] text-stone-400 hidden sm:flex items-center gap-1">
            ...
            >
              ...
              <span>{t('common.close', 'Clear')}</span>
            </button>
          </div>
        </div>
      ...
      <div className="bg-white border border-stone-200/80 rounded-2xl p-4 sm:p-6 shadow-xs min-h-[480px] max-h-[600px] flex flex-col justify-between">
        ...
        <div className="border-t border-stone-100 pt-4 mt-4">
          ...
          <div className="flex items-center justify-between text-[11px] text-stone-400 mt-2 px-1">
            <span>Press Enter to send • 24/7 Crisis Hotline: 14566</span>
