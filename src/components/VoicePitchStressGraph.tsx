...
export const VoicePitchStressGraph: React.FC<VoicePitchStressGraphProps> = ({
  ...
  const speechTimelineData: AudioTimePoint[] = useMemo(() => {
    ...
    let category = activeAssessment ? activeAssessment.stressCategory : (baseScore >= 85 ? 'RISK' : baseScore >= 66 ? 'HIGH' : baseScore >= 36 ? 'MODERATE' : 'LOW');
    if (category === 'MEDIUM') category = 'MODERATE';
    const emotion = activeAssessment?.sentimentMetrics?.primaryEmotion || (category === 'RISK' ? 'Acute Crisis' : category === 'HIGH' ? 'Heightened Distress' : (category === 'MODERATE' || category === 'MEDIUM') ? 'Physical Strain' : 'Calm');

    // Fundamental base frequency (Hz)
    // Low stress: relaxed lower modal register (120-155 Hz)
    // Moderate stress: slightly elevated modal (160-195 Hz)
    // High stress: hyper-adducted tense vocal cords with pitch jumps (210-245 Hz)
    // Risk stress: extreme vocal micro-tremors and hyper-tension (250-320 Hz)
    const baseF0 = category === 'RISK' ? 255 : category === 'HIGH' ? 218 : (category === 'MODERATE' || category === 'MEDIUM') ? 168 : 138;
    ...
    for (let i = 0; i <= totalSteps; i++) {
      ...
      const stressSpike = category === 'RISK'
        ? Math.sin(progress * Math.PI * 2.5) * 16 + (sec > 2.0 && sec < 6.0 ? 14 : 0)
        : category === 'HIGH' 
        ? Math.sin(progress * Math.PI * 2.2) * 12 + (sec > 2.5 && sec < 5.5 ? 8 : 0)
        : (category === 'MODERATE' || category === 'MEDIUM')
        ? Math.sin(progress * Math.PI * 1.8) * 7
      ...
      if (instantStress >= 85) stateDesc = 'Critical Tremor & Distress';
      else if (instantStress >= 66) stateDesc = 'Vocal Tension & Strain';
      else if (pitchCalculated > 210) stateDesc = 'High Pitch Inflection';
      else if (energyEnvelope < 30) stateDesc = 'Micro-Pause / Inhale';
      else if (instantStress <= 35) stateDesc = 'Smooth Resonant Cadence';
      ...
      points.push({
        ...
        emotion,
        state: stateDesc,
        category: instantStress >= 85 ? 'RISK' : instantStress >= 66 ? 'HIGH' : instantStress >= 36 ? 'MODERATE' : 'LOW'
      });
    }
