...
export async function signInWithGoogle(
  ...
  if (!resolvedPhone) {
    throw new Error('Mobile phone number is compulsory for user login.');
  }
  const resolvedAge = age !== undefined && !isNaN(age) ? age : existingProfile?.age;
  const resolvedGender = gender?.trim() || existingProfile?.gender;
  ...
  const profile: UserProfile = {
    ...
    emergencyPhone: resolvedPhone || existingProfile?.emergencyPhone || '14566',
