...
import { getOrCreateUser, getAllUsersFromDb } from './src/db/users';
import { 
  saveAssessmentToDb, 
  getAllAssessmentsFromDb, 
  getAssessmentsByUidFromDb, 
  saveEmergencyAlertToDb 
} from './src/db/assessments';
      ...
      try {
        ...
        if (data.highRiskFlag) {
          await saveEmergencyAlertToDb({
            recipientPhone: data.smsRecipient || '14566',
            recipientName: data.userName || 'High Risk Patient',
            alertMessage: `[HIGH RISK STRESS ALERT] Score: ${data.stressScore}/100. Dispatched via Twilio to 14566.`,
            stressScore: data.stressScore,
            status: data.smsStatus || 'SENT'
          });
        }
      } catch (dbErr) {
        console.warn('Notice: Assessment persistence notice:', dbErr);
      }
    }
...
app.post('/api/chat', async (req, res) => {
  ...
  // - 'complex': gemini-3.1-pro-preview -> gemini-3.8-flash -> gemini-3.1-flash-lite
  // - 'general': gemini-3.8-flash -> gemini-3.1-flash-lite
  // - 'fast': gemini-3.1-flash-lite -> gemini-3.8-flash
  ...
  if (taskType === 'complex') {
    ...
    // For general tasks, prioritize gemini-3.8-flash with gemini-3.1-flash-lite fallback
...
- Emergency Helpline: 14566 available 24/7 if acute crisis.
...
- Emergency Helpline: 14566 available 24/7.
- Current User: ${context?.userName || 'User'} | Stress Score: ${context?.stressScore ?? 'N/A'}/100 (${context?.stressCategory || 'Normal'}).`;
  } else {
...
- Current User: ${context?.userName || 'User'} | Stress Score: ${context?.stressScore ?? 'N/A'}/100 (${context?.stressCategory || 'Normal'}).`;
  ...
  if (context?.languageName && context.languageName !== 'English') {
    systemInstruction += `\n- Language & Localization: The user has chosen to speak in ${context.languageName}. Respond fluently, warmly, and empathetically in ${context.languageName} (using its proper native script, e.g., Hindi in Devanagari, Tamil in Tamil script, Telugu, Kannada, Malayalam, Bengali, Marathi, Gujarati) while keeping clinical and supportive guidance clear.`;
  }
    ...
    try {
      ...
      if (reply) {
        const suggestions = isHigh
          ? ['High Safety Tips & Support', 'Start 4-7-8 Breathing Guide', 'Call Helpline: 14566']
          : taskType === 'fast'
  ...
  if (textCheck.includes('help') || textCheck.includes('crisis') || textCheck.includes('emergency')) {
    fallbackReply = "If you are experiencing overwhelming distress, please know you are supported. Call our 24/7 dedicated helpline at 14566 immediately for direct compassionate care.";
  ...
  return res.json({
    ...
    taskType,
    suggestions: isHigh
      ? ['Start 4-7-8 Breathing Guide', 'Call 14566', 'Try 5-4-3-2-1 Sensory Grounding']
...
app.get('/api/admin/assessments', async (req, res) => {
  try {
    ...
    console.warn('Notice: Using in-memory assessment store fallback:', dbErr);
  }
...
app.get('/api/user/assessments/:userId', async (req, res) => {
  ...
  try {
    ...
    console.warn('Notice: Using in-memory user assessments store fallback:', dbErr);
  }
  ...
  try {
    ...
    if (dbUsers && dbUsers.length > 0) {
      const mapped = dbUsers.map(u => ({
        ...
        registeredAt: u.createdAt ? (typeof u.createdAt === 'string' ? u.createdAt : u.createdAt.toISOString()) : undefined
      }));
      return res.json(mapped);
    }
  } catch (dbErr) {
    console.warn('Notice: Using in-memory users store fallback:', dbErr);
  }
...
app.post('/api/user/sync', async (req, res) => {
  ...
  try {
    ...
    console.warn('Notice: User sync database notice:', dbErr);
  }
...
app.post('/api/admin/test-alert', async (req, res) => {
  const { phone = '14566', name = 'Velayutham S.' } = req.body;
  const formattedTo = toE164(phone || '14566');
