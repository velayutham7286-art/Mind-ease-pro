...
export const FloatingAIChat: React.FC<FloatingAIChatProps> = ({ 
  ...
  const isHighRisk = latestAssessment?.stressCategory === 'HIGH' || latestAssessment?.stressCategory === 'RISK' || (latestAssessment?.stressScore ?? 0) >= 66;
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      suggestions: ['How does voice stress analysis work?', 'Start 4-7-8 Breathing', 'Helpline 14566']
    }
  ]);

  // Proactively react if an assessment changes to HIGH or RISK
  useEffect(() => {
    if (latestAssessment) {
      ...
      if (category === 'HIGH' || category === 'RISK') {
        setMessages((prev) => [
          ...prev,
          {
            id: `alert-${Date.now()}`,
            sender: 'assistant',
            content: `I noticed your recent assessment identified "${emotion || (category === 'RISK' ? 'Critical Acute Distress' : 'Acute Distress')}" with elevated vocal tension (Score: ${score}/100). High Safety Tips & Support have been prioritized for you. Please take a slow, gentle breath with me right now. Remember, you are safe, and you are not alone.\n\nImmediate 24/7 phone support is ready at 14566, and an alert notification was triggered for your emergency contact. Would you like to practice a 2-minute calming grounding technique together?`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            suggestions: [
              'High Safety Tips & Support',
        ...
        setIsOpen(true);
      } else if (category === 'MODERATE' || category === 'MEDIUM') {
        setMessages((prev) => [
          ...prev,
          {
            id: `note-${Date.now()}`,
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const botMsg: ChatMessage = {
        ...
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        suggestions: data.suggestions || (isHighRisk ? ['Start 4-7-8 Breathing', 'Call Helpline 14566'] : ['More relaxation tips', 'View vocal analysis'])
      };
      setMessages((prev) => [...prev, botMsg]);
    } catch (err) {
      // Calming fallback in case of network issue
      const fallbackReply = isHighRisk 
        ? "I am here with you. Take a deep breath: Inhale for 4 seconds, hold for 7, exhale gently for 8. Immediate telephone support is available at 14566."
